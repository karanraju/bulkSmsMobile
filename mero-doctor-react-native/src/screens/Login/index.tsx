...
const LoginScreen = ({navigation}:any) => {
  ...
  const handleLogin = async (values: any) => {
    try {
      ...
      // Bypass server validation - Temporary solution while server is not available
      // Just dispatch the login action directly with the user type
      dispatch(
        logins({userType: values.userType, token: 'dummy-token-for-testing'}),
      );

      console.log('Mock login successful with user type:', values.userType);
    } catch (err) {
      console.error('Failed to login:', err);
    }
  };
